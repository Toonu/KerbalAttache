...
exports.createUser = function createUser(id, nationIn = 'undefined', demonymIn = 'undefined', colorIn = "fffffe", map = 'https://discord.com/') {
    ...
    this.exportFile("config.json", cfg);
...
exports.perm = function perm(message, level, showMessage = true) {
    ...
    if (!clearance && showMessage === true)
        this.messageHandler(message, 'Directorate of Information apologies. Your clearance is not sufficient for this operation. Please contact the moderators if you deem this as an error.');
    return clearance;
};
...
exports.ping = function ping(message, level = 2) {
    ...
    if (message.mentions.users.first()) {
        if (this.perm(message, level, false)) {
            nation = message.mentions.users.first();
        } else {
            this.messageHandler(message, 'You lack sufficient clearance to do this for tagged player. Defaulting to yourself.')
        }
    }
    return nation;
}
...
exports.messageHandler = function messageHandler(message, content, deleteMessage = false,timer = 10000) {
    ...
    message.channel.send(content)
        .then(msg => msg.delete({timeout: timer}).catch(error => this.log(error, true)))
        .catch(networkError => this.log(networkError, true));
    if (deleteMessage) message.delete().catch(error => this.log(error, true));
}
 ...
 */
exports.log = function log(content, erroneous = false) {
    let today = new Date();
 ...
 */
exports.report = function report(message, content, command = '') {
    let today = new Date();
