module.exports = {
    ...
    execute: async function execute(message, args) { 
        ...
        try {
            ...
            if(!units.hasOwnProperty(args[1])) throw 'Asset not found.'
        } catch(err) {
        ...
        gm.findUnitPrice(args[1].toUpperCase(), message, cfg.users[message.author.id].nation, tab)
        .then(data => {
            console.log(data);
            let cost = (parseInt(data[0]) * args[0] * 4);
            ...
            .then(message => {
                ...
                .then(collected => {
                    ...
                    if (react.emoji.name == '✅') {
                        ...
                        let unitAmount;
                        fn.ss(['get', `${data[1]+data[2]}`], message).catch(err => console.error(err))
                        .then(amount => {
